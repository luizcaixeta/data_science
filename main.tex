...
\begin{align}
    I_{G}(p) = \sum_{i = 1}^{J}\left(p_i \sum_{k \diff i}p_k\right) &= \sum_{i = 1}^{J}p_i(1 - p_i) \\
...
crescimento da árvore, como:

\begin{enumerate}
    \item Controle da profundidade máxima da árvore;
    \item Número mínimo de observações em um nó;
    \item Ausência de ganho significativo de informação;
    \item Pureza total do nó.
\end{enumerate}
