...
\section{\textit{Gradient boost for regression}}

O \textit{gradient boost} inicia com uma única folha, que representa
uma estimativa inicial para os pesos de todas as amostras. Com base 
nisso, constrói uma árvore. O próprio \textit{gradient boost} é responsável
por limitar o seu tamanho. Assim como o adaboost, o \textit{gradient boost}
constrói outras árvores baseando-se nos erros cometidos pela árvore 
anterior. Mas ao constrário do adaboost, não constrói somente stumps e 
sim árvores completas de mesmo tamanho e continua o processo até 
consturir a quantidade de árvores setada ou até o momento em que acrescentar
árvores não acresça mais o resultado

Se tivermos um consulto de dados com height, favorite color, gender e 
weight e quisermos desenvolver um modelo de árvore para prever o weight 
de uma pessoa, iniciamos com uma única folha weight que tem como valor 
a média de todos os weights do conjunto de dados. Em seguida, construimos 
uma árvore com base nos erros da primeira. Os erros cometidos pela primeira
árvore são os valores observados - os valores preditos pela folha.
Calculando isso para todos, acrescentamos uma coluna a tabela chamada 
pseudo-residual. Então, uma árvore utilizando height, favorite color 
e gender é construída para prever os pseudo-residuals. Assim, para prever
um valor iniciamos pela folha única e utilizamos a árvore construída para
prever o pseu-residuo, e consideramos como resultado final 
o avarage weight da primeira folha + o residuo previsto pela árvore. Para 
evitar outfiting, o \textit{gradient boost} utiliza uma métrica chamada 
\textit{learning rate} para calibrar a constribuição da nova árvore. O 
\textit{learning rate} é um valor entre 0 e 1. A ideia central do 
\textit{gradient boost} é dar pequenos passos na direção certa, então,
construímos uma nova árvore com base nos erros da primeira. Continuamos 
assim até que reduzamos ao máximo os pseudo-residuos. Matematicamente, o 
algoritmo é:

Para um conjunto de treinamento $\{(x_i, y_i)\}_{i = 1}^{n}$, uma função
de perda diferenciável $\ell (y, F(x))$ e um número de interações $M$, tal
que $1 \leq m \geq M$:

\begin{enumerate}
    \item Inicialize o modelo com um valor constante:
    \begin{equation*}
    ...
    \begin{enumerate}
        ...
        \begin{equation*}
            r_{im} = - \left[ \frac{\partial \ell (y_i, F(x_i))}{\partial F(x_i)}\right]_{F(x) = F_{m - 1}(x)}, \text{for} \ i = 1, ..., n
        \end{equation*}
        \item Ajustar um modelo base fechado sob escalonamento $h_m(x)$ para pseudo-resíduos, 
        ou seja, trein-alo usando o conjunto de treinamento $\{(x_i, r_{im})\}_{i = 1}^{n}$.
        \item Calcular multiplicador $\lambda_m$ resolvendo o seguinte probela de 
        otimização unidimensional:
        \begin{equation*}
            \lambda_m = \arg min_{\lambda} \sum_{i = 1}^{n} \ell (y_i, F_{m - 1}(x_i) + \lambda h_m(x_i))
        \end{equation*}
...
Uma parte importante do gradient boosting é a regularização por encolhimento,
que utiliza uma regra de atualizaão modificada:

\begin{equation}
    F_m(x) = F_{m-1}(x) + v \lamba_m h_m(x), \ 0 < v \geq 1
\end{equation}

Empiricamente, descobriu-se que o uso de taxas de aprendizagem pequenas 
(como $v < 0.1$) produz melhorias drásticas na capacidade de generalização
dos modelos em relação ao gradient boosting sem redução ($v = 1$).
