...
\begin{algorithmic}[1]
\Require Conjunto de dados $D$, número de folds $k$
\Ensure Estimativa de desempenho do modelo
\State Divida $D$ em $k$ folds de tamanho aproximadamente igual: $D_1, D_2, \dots, D_k$
\For{$i = 1$ to $k$}
    \State $D_{\text{treino}} = D \setminus D_i$ \Comment{todos os folds exceto o $i$-ésimo}
    \State $D_{\text{teste}} = D_i$ \Comment{o $i$-ésimo fold}
    \State Treine o modelo em $D_{\text{treino}}$
    \State Avalie o modelo em $D_{\text{teste}}$, registrando a métrica de interesse
\EndFor
\State Calcule a média das métricas registradas para obter a estimativa final
\end{algorithmic}
\end{algorithm}
