...
\begin{equation}
    \textit{Precision} = \frac{\text{VP}}{\text{VP} + \frac{FP}}
\end{equation}
...
\begin{itemize}
    \item Se a \texit{precision} for alta, significa que quando o modelo diz
    que algo é positivo, geralmente acerta
    \item Se a \texit{precision} for baixa, significa que o modelo gera falsos
    positivos.
\end{itemize}

Já o \texit{recall} mede a capacidade do modelo de não gerar falsos 
...
Como vimos, existe uma troca entre \textit{precision} e \textit{recall}. 
Capturar mais exemplos da classe positiva (1) geralmente implica classificar 
incorretamente mais exemplos da classe negativa (0) como positivos.

A métrica que captura essa relação é a curva da ``Característica de Operação 
do Receptor'' (\textit{Receiver Operating Characteristic}), também chamada de 
curva ROC. A curva ROC representa a taxa de verdadeiros positivos 
(\textit{True Positive Rate} ou \textit{recall}) no eixo $y$ em função da taxa 
de falsos positivos (\textit{False Positive Rate}) no eixo $x$.

Ela mostra como o desempenho do classificador varia à medida que o limiar de 
decisão é alterado, evidenciando a troca entre identificar corretamente os 
positivos e evitar falsos alarmes.
