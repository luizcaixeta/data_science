...
Para ilustrar o funcionamento do \textit{gradient boosting} em
problemas de classificação, consideremos um \textit{dataset} com 
as variáveis \textit{likes popcorn}, \textit{age} e \textit{favorite color},
cujo objetivo é prever se uma pessoa gosta ou não do filme \textit{Troll 2}.

Assim como nos problemas de regressão, o algoritmo começa com um 
modelo extremamente simples composto por uma única folha que representa
todo o conjunto de dados. Essa folha fornece uma predição inicial
comum de todas as observações. 

No caso de classificação binária, essa predição inicial é dada pelo
$\log (\text{odds})$, definido como:

\begin{equation*}
    \log \left(\frac{\text{pessoas que amam o filme}}{\text{pessoas que não amam o filme}}\right)
\end{equation*}

Como o modelo precisa produzir probabilidade, transformamos 
o $\log(\text{odds})$ utilizando a função logística:

\begin{equation*}
    P(x) = \frac{e^{\log(\text{odds})}}{1 + e^{\log(\text{odds})}}
\end{equation*}

O valor obtido representa a probabilidade inicial de que uma 
observação pertença à classe $1$ (amar o filme). Para produzir 
uma classificação final, essa probabilidade é comparada ao 
\textit{threshold}. Caso $P(x)$ seja maior que o limiar definido, 
a observação é classificada como $1$; caso contrário, é classificada 
como $0$.

Para observações da classe $1$ (pessoas que amam o filme):

\begin{equation*}
    Residual = 1 - \text{probabilidade prevista}.
\end{equation*}

Para observações da classe $0$ (pessoas que não amam o filme):

\begin{equation*}
    Residual = 0 - \text{probabilidade prevista}.
\end{equation*}

Após calcular esses valores, adicionamos uma coluna de resíduos 
ao \textit{dataset} e treinamos uma árvore de decisão utilizando as 
variáveis \textit{likes popcorn}, \textit{age} e \textit{favorite color}
para prever esses resíduos.

Cada folha dessa nova árvore contém um conjunto de observações com 
resíduos semelhantes. Diferentemente do caso de regressão, não podemos
simplesmente somar os valores das folhas ao modelo existente, pois 
estamos trabalhando no espaço do $\log (\text{odds})$. Assim, o valor 
de saída de cada folha precisa ser ajustado. 

O valor associado a cada folha é calculado como

\begin{equation*}
    \frac{\sum Residual_i}{\sum (p_i (1 - p_i))}
\end{equation*}

em que $p_i$ representa a probabilidade para a observação $i$ 
no modelo atual.

Por fim, atualizamos o modelo somando o valor da prediçaõ anterior 
com o resultado da nova árvore multiplicado pelo \textit{learning rate}.
Esse processo é repetido iterativamente, adicionando novas árvores 
que buscam corrigir os erros do modelo anterior, até que as previsões 
converjam para valores próximos dos resultados observados.

\subsection{Algoritmo}

Para problemas de classificação binária, o \textit{gradient boosting}
utiliza a função de verossimilhança na regressão logística como base 
para definir a função de perda.

A log-verossimilhança para um conjunto de dados com $N$ observações 
é definida como:
...
onde $p_i$ representa a probabilidade prevista de que a observação 
$i$ pertença à classe $1$.

Quanto melhor o modelo se ajustar aos dados, maior será o valor 
da log-verossimilhança. Assim, na regressão logística o objetivo 
é maximizar esse valor. 

Entretando, em problemas de otimização é mais conveniente trabalhar
com uma função de perda a ser minimizada. Para isso, multiplicamos 
a log-verossimilhança por $-1$, obtendo a \textit{log-loss} ou 
\textit{binary cross-entropy}:

\begin{equation*}
    \ell(y,p) = -\sum_{i=1}^{N}\left(y_i \log(p_i) + (1-y_i)\log(1-p_i)\right)
\end{equation*}

Utilizando a relação entre probabilidade e \textit{log-odds}:

\begin{equation}
    F(x) = \log(\text{odds}) = \log\left(\frac{p}{1-p}\right),
\end{equation}

e podemos reescrever a função de perda como:

\begin{equation*}
    \ell(y_i,F(x_i)) = - y_i F(x_i) + \log\left(1 + e^{F(x_i)}\right).
\end{equation*}

A derivada dessa função de perda em relação a $F(x_i)$ é:

\begin{equation*}
    \frac{\partial}{\partial F(x_i)}\ell(y_i,F(x_i)) = - y_i + \frac{e^{F(x_i)}}{1 + e^{F(x_i)}} = - y_i + p_i.
\end{equation*}

E os pseudo-resíduos:

\begin{equation*}
    r_{im} = y_i - p_i
\end{equation*}

Para um conjunto de treinamento $\{(x_i, y_i)\}_{i=1}^{n}$, a função
de perda diferenciável $\ell(y, F(x))$ e um número de iterações $M$:
...
\begin{enumerate}
    ...
    \begin{equation*}
        r_{im} = -\left[\frac{\partial\ell(y_i,F(x_i))}{\partial F(x_i)}\right]_{F(x)=F_{m-1}(x)}.
    \end{equation*}
...
Para obter uma expressão fechada para $\lambda_m$, podemos
aproximiar a função de perda por meio de uma expansão de 
Taylor de segunda ordem em torno de $F_{m - 1}(x_i)$:

\begin{equation*}
    \ell(y_i,F_{m-1}(x_i)+\lambda) \approx \ell(y_i,F_{m-1}(x_i)) + g_i\lambda + \frac{1}{2}h_i\lambda^2,
\end{equation*}

onde:

\begin{equation*}
    g_i = \frac{\partial \ell(y_i,F(x_i))}{\partial F(x_i)}, \qquad h_i = \frac{\partial^2 \ell(y_i,F(x_i))}{\partial F(x_i)^2}.
\end{equation*}

Minimizando essa aproximação quadrática obtemos:

\begin{equation*}
    \lambda_i = -\frac{g_i}{h_i}.
\end{equation*}

No caso específico da regressão logística:
...
\begin{equation*}
    \lambda_i = \frac{y_i - p_i}{p_i(1-p_i)}.
\end{equation*}
...
\section{\textit{Regularization}}

\subsection{Ridge (L2) Regression}

Considere inicialmente o treinamento de um modelo de regressão linear.
O objetivo do modelo é encontrar uma reta (ou hiperplano) que se ajuste 
aos dados minimizando a soma dos quadrados dos resíduos ($SSR$).

QUando o número de observações é pequeno ou quando há muitas variáveis
explicativas, o modelo pode se ajustar excessivamente aos dados de treino.
Nesse caso, a reta ajustada passa muito próxima (ou exatamente) pelos 
pontos observados, capturando inclusive ruídos presentes nos dados.
Esse fenômeno é conhecido como \textit{overfitting} e está associado 
a modelos com \textbf{alta variância}.

A ideia da regularização é introduzir uma penalização que limite o 
tamanho dos coeficientes do modelo. No caso da regressão Ridge, 
adiciona-se um termo de penalização baseado na norma $L_2$ dos 
coeficientes. Isso introduz um pequeno viés (\textit{bias}) na estimativa,
mas reduz a variância do modelo e tende a melhorar sua capacidade de generalização.

Na regressão Ridge, buscamos minizar a seguinte função objetivo:
...
Assim, a função objetivo pode ser escrita como
...
\begin{equation*}
    \beta^T\beta = \beta^T I \beta
\end{equation*}

Portanto,

\begin{equation*}
    \lambda \beta^T \beta = \lambda \beta^T I \beta
\end{equation*}

Assim, podemos agrupar os termos quadráticos em $\beta$:

\begin{equation*}
    \beta^T X^T X \beta + \lambda \beta^T \beta = \beta^T (X^TX + \lambda I)\beta
\end{equation*}

e a função objetivo torna-se

\begin{equation*}
    L(\beta) = Y^TY - 2\beta^TX^TY + \beta^T(X^TX + \lambda I)\beta
\end{equation*}

Derivando em relação a $\beta$:

\begin{equation*}
    \frac{\partial}{\partial \beta} L(\beta) = -2X^TY + 2(X^TX + \lambda I)\beta
\end{equation*}

Para minimizar $L(\beta)$, impomos
...
o que resulta na solução da regressão Ridge:

\begin{equation*}
    \beta_{\lambda} = (X^TX + \lambda I)^{-1}X^TY
\end{equation*}

Uma consequência importante dessa modificação é que, se $X^TX$ possui
autovalores $\lambda_1, \lambda_2, ..., \lambda_p$, então a matriz
regularizada $X^TX + \lambda I$ possui autovalores
...
Isso torna a matriz mais estável numericamente e evita problemas
associados a matrizes quase singulares.

Assim, a regressão Ridge:

\begin{itemize}
\item penaliza coeficientes grandes;
\item reduz a variância do modelo;
\item melhora a capacidade de generalização.
\end{itemize}

O parâmetro de regularização $\lambda$ controla a intensidade da
penalização. Para selecioná-lo, definimos um conjunto de valores
candidatos:

\begin{equation*}
    \lambda \in \{10^{-4}, 10^{-3}, 10^{-2}, 10^{-1}, 0, 1, 10, 100\}
\end{equation*}

Para cada valor de $\lambda$, treinamos o modelo e avaliamos seu
desempenho utilizando \textit{k-fold cross-validation}. O valor
selecionado é aquele que minimiza o erro médio de previsão.

Quando $\lambda = 0$, recuperamos a regressão linear tradicional.
Valores pequenos de $\lambda$ produzem modelos com baixo viés e maior
variância, aumentando o risco de \textit{overfitting}. Por outro lado,
valores grandes de $\lambda$ aumentam o viés, mas reduzem a variância,
resultando em modelos mais estáveis.

Em Python, podemos realizar essa busca automaticamente utilizando
\texttt{RidgeCV}:
...
\subsection{Ridge (L2) for logistic regression}

Na regressão logística modelamos a probabilidade condicional de uma 
observação pertencer à classe $1$ como

\begin{equation*}
    P(Y = 1 \mid X) = \sigma(X\beta),
\end{equation*}

onde $\sigma(\cdot)$ é a função logística definida por

\begin{equation*}
    \sigma(z) = \frac{1}{1 + e^{-z}}.
\end{equation*}

Para um conjunto de dados com $n$ observações, denotamos

\begin{equation*}
    p_i = \sigma(x_i^{T}\beta),
\end{equation*}

que representa a probabilidade prevista para a $i$-ésima observação.

Os parâmetros $\beta$ são estimados minimizando a \textit{negative log-likelihood}
(também chamada de \textit{log loss}):

\begin{equation*}
    L(\beta) = -\sum_{i = 1}^{n}\left[y_i \log(p_i) + (1 - y_i)\log(1 - p_i)\right]
\end{equation*}

Para evitar \textit{overfitting} e controlar a magnitude dos coeficientes,
podemos adicionar uma penalização $L_2$ aos parâmetros. Nesse caso,
a função objetivo passa a ser

\begin{equation*}
    L(\beta) =-\sum_{i = 1}^{n}\left[y_i \log(p_i) + (1 - y_i)\log(1 - p_i)\right] + \lambda ||\beta||^2,
\end{equation*}
...
Diferentemente da regressão linear, em que existe uma solução fechada
para $\beta$ em função de $\lambda$, a regressão logística não possui
uma solução analítica. Assim, os parâmetros são obtidos por métodos
iterativos de otimização, como \textit{gradient descent} ou
\textit{Newton-Raphson}.

Em bibliotecas como o \texttt{scikit-learn}, essa regularização pode ser
...
Nesse caso, o parâmetro $C$ corresponde ao inverso da intensidade da
regularização:
...
Assim, valores pequenos de $C$ (isto é, $\lambda$ grande) implicam
regularização forte, enquanto valores grandes de $C$ ($\lambda$ pequeno) 
correspondem a uma regularização mais fraca.

\subsection{\textit{Lasso (L1) regression}}

A regressão \textit{Lasso} (\textit{Least Absolute Shrinkage and Selection Operator})
é uma técnica de regularização utilizada para reduzir o risco de 
\textit{overfitting} em modelos de regressão. Assim como na regressão
Ridge, a ideia central é adicionar um termo de penalização à função 
de perda do método de mínimos quadrados. No entando, enquanto a regressão 
Ridge utiliza a norma $L_2$, a regressão \textit{Lasso} utiliza a norma
$L_1$.
...
Nesse problema de otimização, o primeiro termo corresponde ao erro 
quadrático residual ($SRR$) do modelos, enquanto o segundo termo 
impõe uma penalização proporcional à soma dos valores absolutos 
dos coeficientes. O parâmetro de regularização $\lambda \geq 0$ 
controla a intensidade dessa penalização. 

Quando $\lambda = 0$, o problema se reduz ao estimador de mínimos 
quadrados ordinários. À medida que $\lambda$ aumenta, os coeficientes 
são progressivamente reduzidos em magnitude.

Uma característica da regressão \textit{Lasso} é que, devido à 
natureza da penalização $L_1$, alguns coeficientes podem ser 
exatamente reduzidos a zero. Como consequência, o \textit{Lasso} 
realiza simultaneamente duas tarefas:

\begin{itemize}
    \item Regularização do modelo, reduzindo a variância das estimativas;
    \item Seleção de variáveis, eliminando preditores irrelevantes.
\end{itemize}

Essa propriedade contrasta com a regressão Ridge, cuja penalização
baseada na norma $L_2$ reduz a magnitude dos coeficientes, mas 
raramente os leva exatamente a zero. 

Na regressão Ridge, o problema de otimização assume a forma 

\begin{equation*}
    \min_{\beta}; |Y - X\beta|^2 + \lambda \sum_{j=1}^{p} \beta_j^2
\end{equation*}

Nesse caso, todos os coeficientes são suavamente reduzidos, 
mas permanecem diferentes de zero. Dessa forma, a regressão 
Ridge melhora a estabilidade numérica do modelo e reduz a variância 
das estimativas, porém não realiza a seleção de variáveis.

A escolha entre Ridge e Lasso depende da estrutura dos dados. A regressão
Ridge tende a apresenta melhor desempenho quando muitas variáveis contribuem 
de forma moderada para a resposta. Por outro lado, a regressão Lasso 
é particularmente útil quando se credita que apenas um subconjunto pequeno 
de variáveis seja realmente relevante para explicar o fenômeno estudado. 

Por essa razão, o Lasso é frequentemente utilizado em problemas de alta 
dimensionalidade, nos quais o número de variáveis preditoras é grande em 
relação ao número de observações.

\section{\textit{XGBoost for regression}}

Cada árvore no \textit{XGBoost} começa com uma única folha, de modo que 
todos os resíduos são iniialmente atribuídos a esse único nó. A partir 
desses resíduos, calculamos uma métrica chamada 
\textit{similarity score}  (ou \textit{quality score}), definida como:

\begin{equation}
    \text{Similarity score} =
    \frac{(\text{Sum of residuals})^2}{\text{Number of residuals} + \lambda}
\end{equation}

onde $\lambda$ é um parâmetro de regularização.

A iedia central é verificar se conseguimos agrupar melhor os resíduos 
dividingo esse único nó em dois grupos. Para isso, escolhemos um valor 
(\textit{threshold}) para uma das variáveis explicativas e realizamos 
um \textit{split}: observações com valores menores que o \textit{threshold}
são enviadas para a folha da esquerda, enquanto observações com valores maiores
são enviadas para a folha da direita.

Após realizar a divisão, calculamos novamente o \textit{similariry score} 
para cada uma das folhas geradas.

Quando os resíduos de um nó possuem sinais ou magnitudes muito diferentes,
eles tendem a se cancelar ao serem somados, o que reduz o valor do 
\textit{similariry score}. Por outro lado, quando os resíduos são mais 
semelhantes, a soma tende a ser maior em magnitude, aumentando o valor 
do \textit{similarity score}. Dessa forma, valores maiores dessa métrica 
indicam que o nó conseguiu agrupar resíduos mais homogêneos.

Para quantificar se a divisão realmente melhora o agrupamento dos
resíduos, calculamos o ganho (\textit{gain}) associado ao \textit{split}:

Cada árvore começa com uma única folha e todos os resíduos
vão para essa única folha. Agora, calculamos o \textit{quality score}
ou \textit{similiraty score} para os resíduos. 
...
Em seguida, avaliamos diferentes valores de \textit{threshold} para a 
mesma variável. Para cada candidato a divisão, repetimos o processo: 
criamos duas folhas, calculamos o \textit{similarity score} de cada uma 
e avaliamos o valor da função ganho.

Esse procedimento continua até avaliarmos todos os possíveis \textit{thresholds}.
O \textit{split} escolhido será aquele que produzir o maior valor de gnaho.

Depois de escolhido o melhor \textit{split}, repetimos o processo em cada 
folha que contém mais de uma observação, consturindo a árvore recursivamente.
No \textit{XGBoost}, por padrão, a profundidade máxima das árvores costuma 
ser limitada (por exemplo, a 6 níveis), o que ajuda a controlar o sobreajuste.

Após a construção da árvore, realizamos um processo de \textit{pruning} baseado 
em um parâmetro $\gamma$. Esse parâmetro define um ganho mínimo necessário 
para manter um ramo na árvore. Para cada divisão, calculamos:

\begin{equation}
    Gain - \gamma
\end{equation}

Se esse valor for negativo, o ramo é removido; caso contrário, o \textit{split}
é matnido. Esse mecanismo evita a criação de divisões que trazem pouca melhoria
ao modelo.

O parâmetro $\lambda$ atua como regularização nos nós da árvore. Valores maiores 
de $\lambda$ reduzem o \textit{similarity score}, penalizando principalmente folhas
com poucas observações. Assim, folhas contedo apenas uma observação são mais 
fortemente penalizadas do que folhas com muitos dados.

O \textit{output value} de cada folha é dado por:

\begin{equation}
    Output\ value =
    \frac{\text{Sum of residuals}}
    {\text{Number of residuals} + \lambda}
\end{equation}

Quando $\lambda > 0$, a contribuição da folha para a previsão final é 
reduzida, tornando o modelo menos sensíveis para observações individuais.

Por fim, o \textit{XGBoost} atualiza as previsões combinando a previsão
atual com a contribuição da nova árvore encontrada:

\begin{equation}
    \hat{y}_{new} =
    \hat{y}_{old} +
    \eta \cdot (\text{prediction from tree})
\end{equation}

onde $\eta$ é o \textit{learning rate}. O valor padrão é $0.3$.

Apósessa atualização, novos resíduos são calculados e uma nova árvore
é construída para corrigir os erros restantes. Esse processo é repetido 
iterativamente, de modo que cada nova árvore realiza um pequeno passo 
na direção de reduzir os resíduos do modelo.

\section{\textit{XGBoost for classification}}

Para problemas de classificação binária, o algoritmo também inicia 
com uma única folha. Nesse caso, a predição inicial atribui a mesma 
probabilidade para todas as amostras. Quando não há informação prévia, 
essa probabilidade inicial é tipicamente $0.5$, o que equivale a uma 
classificação aleatória.

A partir dessa estimativa inicial, calculamos os resíduos (que, no caso 
da classificação logística, correspondem ao gradiente da função de perda) e
ajustamos uma árvore para esses valores. Em seguida, calculamos o 
\textit{similarity score} de cada nó da árvore. Para classificação, ele é
definido como:

\begin{equation}
\text{Similarity score} =
\frac{\left(\sum_i \text{Residual}_i\right)^2}
{\sum_i \left[p_i (1 - p_i)\right] + \lambda}
\end{equation}

onde $p_i$ representa a probabilidade estimada para a observação $i$, e 
$\lambda$ é um parâmetro de regularização.

Assim como no caso de regresão, começamos com todos os resíduos em um 
único nó. Em seguida, testamos possíveis \textit{thresholds} para dividir 
os dados em dois grupos, buscando aumentar o \textit{similariry score}. 
Para cada divisão candidata, calculamos novamente esse valor nos nós 
resultantes.

A qualidade da divisão é medida pela função de ganho (\textit{gain}):

\begin{equation}
\text{Gain} =
\text{Similarity}_{left} +
\text{Similarity}_{right} -
\text{Similarity}_{root}
\end{equation}

Se o ganho for positivo, a divisão melhora o modelo e pode ser mantida.

Outro conceito importante é o \textit{cover}, que mede a quantidade efetiva
de informação presente em um nó. ELe é definido como

\begin{equation}
\text{Cover} = \sum_i p_i (1 - p_i)
\end{equation}

Esse valor funciona como uma medida do número efetivo de observações no nó.
O \textit{XGBoost} exige que cada folha tenha um \textit{cover} mínimo; caso
contrário o nó é removido (\textit{pruned}). O controle desse critério é 
feito pelo hiperparâmetro \textit{min\_child\_weight}.

O processo de \textit{pruning} segue a mesma lógica utilizada na regressão.
Definimos um hiperparâmetro $\gamma$ que representa o ganho mínimo necessário 
para manter uma divisão. Assim, subtraímos $\gamma$ do valor de \textit{gain};
se o resultado for positivo, a divisão é mantida, caso contrário o nó é 
eliminado.

Por fim, a predição final do modelo é obtida somando-se as constribuições de 
todas as árvores. No caso da classificação, cada árvore produz uma correção 
no valor do \textit{log-odds}. Assim, a atualização da predição pode ser escrita
como

\begin{equation}
\text{log-odds}_{new} =
\text{log-odds}_{previous} +
\eta \times \text{output}_{tree}
\end{equation}

onde $\eta$ é o \textit{learning rate}. O valor final de probabilidade é então 
obtido aplicando a função logística ao \textit{log-odds}.

\subsection{Algoritmo}

Considere um conjunto de treinamento $\{(x_i, y_i)\}_{i=1}^{N}$, uma função de perda
diferenciável $\ell(y, F(x))$, um número de árvores fracas $M$ e uma taxa de
aprendizado $\alpha$.
...
\begin{equation}
\hat{f}_{(0)} = \arg \min_{\theta} \sum_{i=1}^{N} \ell(y_i, \theta)
\end{equation}

Esta etapa define a predição inicial do modelo. Como nenhuma árvore foi
construída ainda, o modelo retorna o mesmo valor para todas as entradas.
Assim, escolhemos a constante $\theta$ que minimiza a função de perda
sobre todo o conjunto de treinamento.
...
\item Calcular os gradientes e hessianos da função de perda avaliados no
modelo atual:

\begin{align*}
\hat{g}_i &= 
\left[\frac{\partial \ell(y_i, f(x_i))}{\partial f(x_i)}\right]_{f(x)=\hat{f}_{(m-1)}(x)} \\
\\
\hat{h}_i &=
\left[\frac{\partial^2 \ell(y_i, f(x_i))}{\partial f(x_i)^2}\right]_{f(x)=\hat{f}_{(m-1)}(x)}
\end{align*}

Os termos $\hat{g}_i$ e $\hat{h}_i$ representam, respectivamente, o
gradiente e a segunda derivada da função de perda em relação à predição
do modelo para a amostra $i$. Esses valores fornecem uma aproximação
quadrática da função de perda ao redor do modelo atual.

\item Ajustar um aluno base (por exemplo, uma árvore de decisão)
utilizando os pares

\[
\left\{x_i,\; -\frac{\hat{g}_i}{\hat{h}_i}\right\}_{i=1}^{N}
\]
...
\frac{1}{2}\hat{h}_i
\left[
\phi(x_i) + \frac{\hat{g}_i}{\hat{h}_i}
\right]^2
\end{equation}

Esse problema corresponde a ajustar uma função $\phi(x)$ que aproxime
o passo de atualização obtido pela expansão de segunda ordem da
função de perda.

\item Definir a contribuição da nova árvore:

\begin{equation}
\hat{f}_m(x) = \alpha \hat{\phi}_m(x)
\end{equation}

onde $\alpha$ é a taxa de aprendizado que controla o tamanho do passo
de atualização.
...
\hat{f}(x) =
\hat{f}_{(M)}(x) =
\sum_{m=0}^{M} \hat{f}_m(x)
\end{equation}

\end{itemize}

Para entender como o XGBoost determina os valores preditos em cada folha
da árvore, considere uma folha $j$ que contém um conjunto de amostras
$I_j$.

Queremos encontrar o valor constante $w_j$ associado a essa folha que
minimiza a função de perda aproximada. Utilizando a expansão de segunda
ordem da função de perda, o problema de otimização local pode ser escrito
como
...
\begin{itemize}
\item $g_i$ é o gradiente da função de perda para a amostra $i$
\item $h_i$ é o hessiano da função de perda para a amostra $i$
\end{itemize}

Como $w_j$ é constante para todas as amostras da folha, podemos escrever

\begin{equation}
\mathcal{L}_j(w_j) =
\left(\sum_{i \in I_j} g_i\right) w_j +
\frac{1}{2}
\left(\sum_{i \in I_j} h_i\right) w_j^2
\end{equation}

Para encontrar o valor ótimo de $w_j$, derivamos em relação a $w_j$ e
igualamos a zero:
...
\sum_{i \in I_j} g_i
+
\left(\sum_{i \in I_j} h_i\right) w_j
= 0
\end{equation}

Resolvendo para $w_j$ obtemos
...
Esse valor corresponde à predição atribuída à folha $j$ da árvore.

Substituindo o valor ótimo de $w_j$ na função de perda aproximada,
obtemos o valor mínimo da perda para aquela folha:
...
Esse valor é utilizado pelo algoritmo para avaliar a qualidade de uma
folha da árvore. Na literatura do XGBoost, essa quantidade é conhecida
como \textit{similarity score}.
...
Intuitivamente, esse valor mede o quanto os gradientes dentro da folha
estão alinhados. Folhas com valores altos de \textit{similarity score}
indicam que as amostras apresentam gradientes consistentes e que a
folha pode reduzir significativamente a função de perda.

Durante a construção da árvore, o algoritmo avalia possíveis divisões
calculando o ganho (\textit{gain}) obtido ao dividir uma folha em duas
subfolhas:

\begin{equation}
\text{Gain} =
\frac
{\left(\sum g_L\right)^2}
{\sum h_L}
+
\frac
{\left(\sum g_R\right)^2}
{\sum h_R}
-
\frac
{\left(\sum g_P\right)^2}
{\sum h_P}
\end{equation}

onde $L$, $R$ e $P$ representam respectivamente as folhas esquerda,
direita e a folha original antes da divisão.

O algoritmo escolhe a divisão que produz o maior ganho na redução
da função de perda.
